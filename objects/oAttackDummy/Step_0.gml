...
if (distance_to_object(oMaya) <= detection_radius)
{
    
    show_debug_message("Maya is within radius!");
...
else if (distance_to_object(oVenture) <= detection_radius) {
	show_debug_message("Venture is within radius!");
	if(allow_shoot == true) && instance_exists(oMaya){
	_angle = point_direction(x,y,oMaya.x,oMaya.y)
	direction = _angle
	var effect_x = x + lengthdir_x(30, _angle);
...
{
    // Code to execute if obj_enemy is not within the radius
    show_debug_message("Maya is outside the radius.");
}
